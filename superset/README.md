# Superset — virtual employee orchestrator

WhatsApp (Meta Cloud API) <-> this orchestrator <-> headless Claude Code <-> GitHub, restricted to
opening PRs against each repo's staging branch. One task runs at a time; anyone who messages while
Superset is busy gets told what it's working on and gets a reply once it's free. The person who
started the current task can interrupt it by sending "stop".
...
3. If the message is "stop"/"cancel"/"abort" (optionally followed by a new instruction, e.g.
   ...
   - from whoever started the currently-running task -> the in-flight Claude Code process is
     ...
     queue and runs next, otherwise Superset just goes idle.
   - from anyone else -> they're told only the task's owner can stop it.
4. Otherwise, if Superset is mid-task, the message is queued and the sender is told what's in
   progress.
...
Superset does not treat each WhatsApp message as an isolated task. Each **project** has its own
...
Superset runs with no human present, so nothing is allowed to block on a prompt:
...
5. Set `ALLOWED_SENDER_NUMBERS` to the E.164 digits (no "+") of every number allowed to talk to
   Superset - anyone else is invisible to it.

### 2. GitHub bot identity

1. Create a dedicated GitHub account for the bot (e.g. `superset-bot`) and add it as a collaborator
...
3. **Protect `main`** on every repo (Settings → Branches → branch protection rule): require a pull
   ...
   account admin/bypass rights. This is what actually stops Superset from ever landing on `main`,
...
npm install
npm run build
sudo cp deploy/superset.service /etc/systemd/system/superset.service
...
- **Restart mid-task**: the in-flight job and the in-memory queue are dropped on restart (a
  ...
  resume - but the requester of an interrupted job isn't auto-notified; check `data/superset.log`.
